...
data_short<-combine %>% dplyr::select(nat_species, introduced_precip_niche,
                                      ...
                                      intro_abs_med_lat, n, nat_abs_med_lat,
...
dom_temp<-data_melt %>% 
  subset(variable=="introduced_temp_niche" | variable=="nat_temp_range") %>% 
  group_by(nat_Domatia) %>%
  #summarise(average = mean(value)) %>% 
  ggplot()+
  aes(x=variable, y=value, fill=nat_Domatia)+
  geom_boxplot(stat="boxplot")+
  theme_classic()+
  xlab("Domatia")+
...
P<-cowplot::plot_grid(EFN_temp, dom_temp, fixer_temp,
                      EFN_precip, dom_precip, fixer_precip,
                      EFN_nitro, dom_nitro, fixer_nitro,
